# Plan 003: Preserve originals when composition verification fails
...
## Why this matters
composeFrame archives originals before audit/export. An export rejection enters a catch which removes only the replacement root. The reported failure therefore leaves the original design hidden and its replacement deleted.
...
2. Add regressions: Extend the existing compose rollback test at test/plugin-logic.test.mjs:307 with requested archives and export rejection. Also inject audit failure, metadata failure after grouping, and verify successful archive. Assert originals' parent/order/visibility/position, no lost originals or stray new nodes, and correct surviving replacement references.
...
3. Implement: Perform fallible audit/export before archiving originals. Make the archive commit recoverable: capture original parent, sibling order, visibility and position/transform before grouping; if grouping or metadata assignment or later response preparation fails, restore originals and remove only new archive/replacement nodes. Avoid deleting an archive while it still contains originals. Preserve native composition, collision policy, hidden archive semantics, and returned relationship metadata. Prepare fallible output before committing where practical.
...
## STOP conditions
Report to reviewer if target behavior has drifted, dependencies cannot install, the same gate fails twice after a reasonable fix attempt, or out-of-scope edits are necessary. If preserving original Figma transforms/order needs an unverified API, verify official documentation before proceeding; report if the recovery contract cannot be implemented within scope.
