# Plan 005: Remove newly created instances after failed configuration
...
## Why this matters
createComponentInstance creates a node before attaching it and applying properties. A rejected parent or property leaves an unwanted instance behind despite a failed MCP result; repeated attempts accumulate duplicates.
...
2. Add regressions: Model tests after the existing successful component-instance test at test/plugin-logic.test.mjs:266. Cover parent rejection, invalid property failure, later configuration/result failure, and success. Assert exactly the new instance is removed on failure and linked source/parent/unrelated nodes survive.
...
3. Implement: Wrap configuration and result preparation after createInstance in the existing removeFailedCreation cleanup pattern. Clean up only this newly created instance; never remove the source component, imported definition, parent, or unrelated nodes. Preserve verified-ID/key lookup and successful return shape.
...
## STOP conditions
Report to reviewer if target behavior has drifted, dependencies cannot install, the same gate fails twice after a reasonable fix attempt, or out-of-scope edits are necessary. Library management, component creation tooling, or deleting imported definitions is outside scope.
